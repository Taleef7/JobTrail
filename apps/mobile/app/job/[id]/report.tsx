...
function generateReportText(
  ...
  if (approvals.length > 0) {
    lines.push('Customer Approval:');
    approvals.forEach(a => {
      lines.push(`  Approved by: ${a.customerName || 'Unknown'}`);
      if (a.approvedAt) lines.push(`  Date: ${new Date(a.approvedAt).toLocaleDateString()}`);
      if (a.approvalNotes) lines.push(`  Notes: ${a.approvalNotes}`);
    });
    lines.push('');
  }
...
export default function ReportPreviewScreen() {
  ...
  const handleShare = async () => {
    const text = generateReportText(job, notes, materials, timeEntries, extractedWorkPerformed, extractedFollowUps, approvals);
    try {
      ...
      {approvals.length > 0 && (
        <View style={styles.section}>
          ...
          {approvals.map((approval) => (
            <View key={approval.id} style={styles.bulletItem}>
              <Text style={styles.itemText}>
                {approval.customerName || 'Unknown'} — Approved {approval.approvedAt ? formatDate(approval.approvedAt) : ''}
              </Text>
              {approval.approvalNotes && (
                <Text style={styles.itemMeta}>{approval.approvalNotes}</Text>
              )}
            </View>
          ))}
        </View>
